<template>
    <a-modal :visible="value" :title="modalTitle" :maskClosable="false"
        ...
        <a-form id="form" :form="form" ref="form">
            ...
            <a-form-item label="所属镇（街道）">
                <town-refer :sync="value" v-decorator="['parentId', rules.parentId]"/>
            </a-form-item>
        </a-form>
    </a-modal>
</template>
...
<script>
    import TownRefer from "@/views/platform/bd/addr/town/refer"
    import rules from '../rules'

    export default {
        name: "EditModal",

        components: {TownRefer},
        ...
        computed: {
            modalTitle() {
                return this.modalType === 'add' ? '新增村（社区）' : '修改村（社区）'
            }
        },
