<template>
    <a-select :value="value"
        ...
        <a-select-option v-if="village" :key="village.id" :value="village.id">
            {{village.name}}
        </a-select-option>
        <div slot="dropdownRender" slot-scope="menu">
            <a-tabs default-active-key="1" v-model="activeTabKey"
                ...
                <a-tab-pane key="4" :tab="town ? town.title : '镇/街道' " :disabled="towns.length === 0">
                    <div class="tab-pane">
                        <a-row :gutter="[8, 8]">
                            <template v-for="town in towns">
                                <a-col :span="8">
                                    <a @click="onTownClick(town)"> {{town.title}} </a>
                                </a-col>
                            </template>
                        </a-row>
                    </div>
                ...
                <a-tab-pane key="5" :tab="village ? village.title : '村/社区' " :disabled="villages.length === 0">
                    <div class="tab-pane">
                        <a-row :gutter="[8, 8]">
                            <template v-for="village in villages">
                                <a-col :span="8">
                                    <a @click="onVillageClick(village)"> {{village.title}} </a>
                                </a-col>
                            </template>
                        </a-row>
                    </div>
...
<script>
    ...
    import townService from '@/views/platform/bd/addr/town/service'
    import villageService from '@/views/platform/bd/addr/village/service'
    ...
    export default {
        ...
        methods: {
            ...
            onVillageClick(village) {
                if (!this.village || this.village.id !== village.id) {
                    this.onChange(village.id)
                } else {
            ...
            async fetchTowns() {
                const towns = await townService.fetchAll({parentId: this.district.id})
                arraySort(towns, 'code')
                this.towns = towns
            },

            async fetchVillages() {
                const villages = await villageService.fetchAll({parentId: this.town.id})
                arraySort(villages, 'code')
                this.villages = villages
            },

            async fetchVillage() {
                if (this.value) {
                    this.village = await villageService.fetchOne(this.value)
                    if (this.village) {
                        this.town = await townService.fetchOne(this.village.parentId)
                        if (this.town) {
                            this.district = await districtService.fetchOne(this.town.parentId)
                            if (this.district) {
