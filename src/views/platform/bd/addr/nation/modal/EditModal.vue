<template>
    <a-modal :visible="value" :title="modalTitle" :maskClosable="false" centered
             @cancel="onCancel">
        ...
        <a-form id="form" :form="form" ref="form">
            <a-row :gutter="8">
                <a-col :span="12">
                    <a-form-item label="国家（地区）编码">
                        <a-input v-decorator="['code', rules.code]" autoComplete="off"/>
                    </a-form-item>
                </a-col>
                <a-col :span="12">
                    <a-form-item label="国家（地区）简称">
...
<script>
    ...
    export default {
        ...
        computed: {
            modalTitle() {
                return this.modalType === 'add' ? '新增国家（地区）' : '修改国家（地区）'
            }
        },
        ...
        watch: {
            value(visible) {
                if (!visible) {
                    ...
                    const {code, title, name, zip} = this.modalData || {}
                    const values = {code, title, name, zip}
                    this.$nextTick(() => {
