<template>
    <a-modal :visible="value" :title="modalTitle" :maskClosable="false" :destroyOnClose="true"
             @cancel="onCancel">
        ...
        <a-form id="form" :form="form" ref="form">
            <a-row :gutter="8">
                <a-col :span="12">
                    <a-form-item label="市（市辖区）编码">
                        <a-input v-decorator="['code', rules.code]" autoComplete="off"/>
                    </a-form-item>
                </a-col>
                <a-col :span="12">
                    <a-form-item label="市（市辖区）简称">
            ...
            <a-form-item label="所属省（直辖市）">
                <province-refer v-decorator="['parentId', rules.parentId]"/>
            </a-form-item>
        </a-form>
    </a-modal>
</template>
...
<script>
    import ProvinceRefer from "@/views/platform/bd/addr/province/refer/ProvinceRefer"
    import rules from '../rules'

    export default {
        name: "EditModal",

        components: {ProvinceRefer},
        ...
        computed: {
            modalTitle() {
                return this.modalType === 'add' ? '新增市（市辖区）' : '修改市（市辖区）'
            }
        },
