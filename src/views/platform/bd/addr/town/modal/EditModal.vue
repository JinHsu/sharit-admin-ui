<template>
    <a-modal :visible="value" :title="modalTitle" :maskClosable="false"
        ...
        <a-form id="form" :form="form" ref="form">
            ...
            <a-form-item label="所属县（区）">
                <district-refer :sync="value" v-decorator="['parentId', rules.parentId]"/>
            </a-form-item>
        </a-form>
    </a-modal>
</template>
...
<script>
    import DistrictRefer from "@/views/platform/bd/addr/district/refer"
    import rules from '../rules'

    export default {
        name: "EditModal",

        components: {DistrictRefer},
        ...
        computed: {
            modalTitle() {
                return this.modalType === 'add' ? '新增镇（街道）' : '修改镇（街道）'
            }
        },
