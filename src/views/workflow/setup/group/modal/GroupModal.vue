<template>
    <a-modal :visible="value" :title="modalTitle" :maskClosable="false"
        ...
        <a-form id="form" :form="form" ref="form">
            ...
            <a-row>
                <a-col>
                    <a-form-item label="上级流程用户组">
                        <group-refer :sync="value" v-decorator="['parentId']"/>
                    </a-form-item>
                </a-col>
            </a-row>
            <a-row>
...
<script>
    import GroupRefer from "../refer"
    import rules from '../rules'

    export default {
        name: "GroupModal",
        components: {GroupRefer},
        props: {
            value: {
                type: Boolean,
                default: false
        ...
        computed: {
            modalTitle() {
                return this.modalType === 'add' ? '新增流程用户组' : '修改流程用户组'
            },
            isEdit() {
